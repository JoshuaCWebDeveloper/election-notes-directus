...
function setUpChart() {
    const metrics = props.data
        .map(metric => {
            ...
            const yValue = props.function
                ? Number(metric?.[props.function]?.[props.yAxis])
                : Number(metric?.[props.yAxis]);
            const y =
                props.decimals >= 0 ? yValue.toFixed(props.decimals) : yValue;
    ...
    function getFillColor(x: string | number, y: string | number) {
        ...
        if (isNil(x) || isNil(y) || !props.conditionalFill?.length)
            return fillColor;
