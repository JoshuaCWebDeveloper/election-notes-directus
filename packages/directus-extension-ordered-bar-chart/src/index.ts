...
export default definePanel({
    id: 'ordered-bar-chart',
    name: 'Ordered Bar Chart',
    description: 'A panel that displays a bar chart with ordered data',
    icon: 'bar_chart',
    ...
    query: options => {
        ...
        return {
            ...
            query: {
                sort: [`${options['sortOrder']}${options['sortField']}`],
                group: [options['xAxis'], options['sortField']],
    ...
    options: ({ options }) => {
        ...
        const needsANumber = computed(() => {
            if (
                !options?.['function'] ||
                !['count', 'last', 'first'].includes(options['function'])
            )
                return true;
            return false;
        });
        ...
        return [
            ...
            },
            {
                ...
                meta: {
                    ...
                    options: {
                        template:
                            '{{color}} {{axis}}-Axis {{operator}} {{value}}',
                        fields: [
                            {
                                field: 'axis',
                                name: '$t:axis',
